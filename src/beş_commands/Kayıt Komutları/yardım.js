...
module.exports = {
    ...
    let commandsFive = client.commands.filter(bes => bes.usage).map((fivesobes) => `> \`${beş_config.prefix}${fivesobes.usage}\``).join("\n");
     ...
     message.reply({ embeds: [beş_embed.setDescription(`${commandsFive}`).setThumbnail(message.guild.iconURL({dynamic:true})).setTitle(`Yardım Menüsü`).setURL(`https://discord.gg/yaztatili`)] });
